...
local function keyForm(keyCode)
    ...
    if(formView==1)then
        if(keyCode==KEY_1)then
            -- open a log file
            filebox.openfile(128,"Choose log file","/log","/log",{"log"},newFile,formView)
        end
        
        if(keyCode==KEY_2)then
            -- open a file in /Apps folder
            filebox.openfile(128,"Choose app","/Apps","/",{"*"},newFile,formView)
        end
...
local function initForm(subform)
    ...
    if(subform==1)then
        ...
        form.addLink((function()filebox.openfile(128,"Choose file","/","/",{"wav","mp3"},newFile,subform) end),{label = "open file.."})
